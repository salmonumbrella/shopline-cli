...
func newTagsTestCmd() *cobra.Command {
	...
	cmd.Flags().String("query", "", "") // For tags list query filter (different from root jq query)
	return cmd
}
...
func TestTagsListRunE_GetClientFails(t *testing.T) {
	...
	cmd.Flags().Int("page", 1, "")
	cmd.Flags().Int("page-size", 20, "")
	// Note: The tagsListCmd expects a "query" flag which is different from root's "query" (jq filter)
