...
var flashPriceCampaignsCreateCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if dryRun {
			_, _ = fmt.Fprintln(formatterWriter, "[DRY-RUN] Would create flash price campaign")
			return nil
		}
...
var flashPriceCampaignsUpdateCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		dryRun, _ := cmd.Flags().GetBool("dry-run")
		if dryRun {
			_, _ = fmt.Fprintln(formatterWriter, "[DRY-RUN] Would update flash price campaign")
			return nil
		}
...
var flashPriceCampaignsDeleteCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		dryRun, _ := cmd.Flags().GetBool("dry-run")
		if dryRun {
			_, _ = fmt.Fprintln(formatterWriter, "[DRY-RUN] Would delete flash price campaign")
			return nil
		}
