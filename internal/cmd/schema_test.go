	...
	tests := []struct {
	...
	}{
		{
			...
			wantContains: []string{
				...
				"ID Prefix: [order:$id]",
				"Commands:",
				"shopline test-orders list",
				"shopline test-orders get",
				"shopline test-orders cancel",
			},
		},
		{
			name:         "json output",
...
func TestSchemaErrorMessage(t *testing.T) {
	...
	if !strings.Contains(errMsg, "shopline schema") {
		t.Errorf("error should contain hint 'shopline schema', got: %v", errMsg)
	}
}
