...
func TestCustomerSavedSearchesCreateRunE_GetClientFails(t *testing.T) {
	...
	cmd := newTestCmdWithFlags()
	cmd.Flags().String("name", "VIP Customers", "")
	_ = cmd.Flags().Set("query", "orders_count > 10")
...
// This avoids conflicts with the JQ filter "query" flag.
...
// Note: The create command has a flag conflict - the "query" flag is used both for
// the search query value AND for JQ filtering by getFormatter.
// We use the identity JQ filter "." to avoid interpretation issues.
func TestCustomerSavedSearchesCreateRunEWithJSON(t *testing.T) {
	...
	// Use "." which is a valid JQ identity filter
...
func TestCustomerSavedSearchesCreateMultipleProfiles(t *testing.T) {
	...
	_ = cmd.Flags().Set("query", "test_query")
	err := customerSavedSearchesCreateCmd.RunE(cmd, []string{})
	if err == nil {
