...
func TestOperationLogsAliases(t *testing.T) {
	expectedAliases := []string{"audit-logs", "audit", "logs"}
...
func TestOperationLogsGetRunE(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// For text output, the get command uses fmt.Printf, so we need to capture stdout
			var output string
			if tt.outputFormat != "json" && !tt.wantErr {
				oldStdout := os.Stdout
				r, w, _ := os.Pipe()
				os.Stdout = w
				...
				// For JSON output, check the formatter buffer
				output = buf.String()
			}
