...
func EnrichError(err error, resource, resourceID string) error {
	...
	case errors.As(err, &authErr):
		...
		rich.Suggestions = []string{
			"Run 'shopline auth login' to re-authenticate",
	...
	case errors.As(err, &validationErr):
		...
		rich.Suggestions = []string{
			...
			fmt.Sprintf("Run 'shopline schema %s' for field requirements", resource),
		}
	...
	case errors.As(err, &apiErr):
		...
		case 404:
			...
			rich.Suggestions = []string{
				...
				fmt.Sprintf("Run 'shopline %s list' to see available %s", resource, resource),
			}
		case 409:
			rich.Code = "CONFLICT"
			rich.Suggestions = []string{
