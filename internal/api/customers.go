...
// CustomerStoreCreditUpdateRequest contains the request body for updating store credits.
type CustomerStoreCreditUpdateRequest struct {
...
// UpdateCustomerStoreCredits updates store credits for a customer.
func (c *Client) UpdateCustomerStoreCredits(ctx context.Context, id string, amount float64, reason string) (*Customer, error) {
	if strings.TrimSpace(id) == "" {
		return nil, fmt.Errorf("customer id is required")
	}
