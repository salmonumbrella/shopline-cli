...
func TestUpdateCustomerStoreCredits(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if req.Amount != 50.00 {
			t.Errorf("Unexpected amount: %f", req.Amount)
		}
		if req.Reason != "Loyalty bonus" {
	...
	customer, err := client.UpdateCustomerStoreCredits(context.Background(), "cust_123", 50.00, "Loyalty bonus")
	if err != nil {
...
func TestUpdateCustomerStoreCreditsNegativeAmount(t *testing.T) {
	...
	_, err := client.UpdateCustomerStoreCredits(context.Background(), "cust_123", -25.50, "Refund deduction")
	if err != nil {
		t.Fatalf("UpdateCustomerStoreCredits failed: %v", err)
	}
}
...
func TestUpdateCustomerStoreCreditsEmptyID(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			_, err := client.UpdateCustomerStoreCredits(context.Background(), tc.id, 10.00, "test")
			if err == nil {
				t.Error("Expected error for empty ID, got nil")
			}
			if err != nil && err.Error() != "customer id is required" {
...
func TestUpdateCustomerStoreCreditsAPIError(t *testing.T) {
	...
	_, err := client.UpdateCustomerStoreCredits(context.Background(), "cust_123", -1000.00, "Large deduction")
	if err == nil {
		t.Error("Expected error for API error response, got nil")
	}
}
